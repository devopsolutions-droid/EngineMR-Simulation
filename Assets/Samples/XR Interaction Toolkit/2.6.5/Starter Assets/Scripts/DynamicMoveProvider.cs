...
namespace UnityEngine.XR.Interaction.Toolkit.Samples.StarterAssets
{
    ...
    public class DynamicMoveProvider : ActionBasedContinuousMoveProvider
    {
        ...
        private static System.Reflection.PropertyInfo s_IsGrabModeActiveProperty;
        private static bool s_CheckedProperty;

        private static bool IsGrabModeActiveReflection()
        {
            if (!s_CheckedProperty)
            {
                s_CheckedProperty = true;
                try
                {
                    var type = System.Type.GetType("EngineViewManager, Assembly-CSharp");
                    if (type != null)
                    {
                        s_IsGrabModeActiveProperty = type.GetProperty("IsGrabModeActive", 
                            System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Static);
                    }
                }
                catch (System.Exception ex)
                {
                    Debug.LogWarning($"[DynamicMoveProvider] Failed to reflect EngineViewManager: {ex.Message}");
                }
            }

            if (s_IsGrabModeActiveProperty != null)
            {
                try
                {
                    return (bool)s_IsGrabModeActiveProperty.GetValue(null);
                }
                catch
                {
                    return false;
